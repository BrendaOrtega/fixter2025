...
export const loader = async ({ request, params }: Route.LoaderArgs) => {
  ...
  if (sequenceVideos.length) {
    ...
    if (sequenceId) {
      ...
      if (seq) {
        sequenceUrl = `/secuencias/${seq.slug || seq.id}`;
...
export default function Route({
  ...
  return (
    <>
      ...
      {showPurchaseDrawer && <PurchaseDrawer key={video.id} courseSlug={course.slug} price={course.basePrice} courseTitle={course.title} kind={course.tipo} stage={course.stage} unlockedCount={videos.filter((v: any) => v.accessLevel === "paid").length} totalSessions={course.totalSessions} duration={course.duration} />}
