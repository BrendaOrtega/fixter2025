#!/usr/bin/env npx tsx
/**
 * Recordatorio de la sesión 5 ("Canales + WhatsApp"), la última, a los alumnos del taller
 * "Diseño de sistemas agénticos", con el recorrido de las cinco sesiones.
 *
 * Se manda el mismo día de la sesión, por la mañana. Va a los inscritos del
 * curso, no a la lista de marketing.
 *
 * Uso:
 *   npx tsx --env-file=.env scripts/send-taller-sesion-5.ts                  # dry-run
 *   npx tsx --env-file=.env scripts/send-taller-sesion-5.ts --prueba --send  # a mis cuentas
 *   npx tsx --env-file=.env scripts/send-taller-sesion-5.ts --send           # a los alumnos
 *   npx tsx --env-file=.env scripts/send-taller-sesion-5.ts --only x@y.com --send
 */
import { db } from "../app/.server/db";
import { sendTallerSession5Cierre } from "../app/mailSenders/sendTallerSession5Cierre";
...
/** Enviado el 14 sep 2026 a los 10 inscritos. Si el script se vuelve a correr
 *  sin esta lista, les llega dos veces. */
...
async function main() {
  ...
  for (const u of lista) {
    ...
    if (!send) continue;
    await sendTallerSession5Cierre({ to: u.email, userName: nombre });
  }
