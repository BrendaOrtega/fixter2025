...
async function main() {
  ...
  const existingBySlug = await db.post.findUnique({
    where: { slug: "Que-es-un-agente-AI_elw" },
  });

  if (existingBySlug) {
    console.log("⚠️  El post ya existe con slug 'Que-es-un-agente-AI_elw'");
    console.log(`   ID: ${existingBySlug.id}`);
    console.log(`   URL: /blog/${existingBySlug.slug}`);
    return;
  }

  // Verificar por título (hay constraint unique en title)
  const existingByTitle = await db.post.findFirst({
    where: { title: "¿Qué es un agente AI?" },
  });
  ...
  if (existingByTitle) {
    ...
    console.log("\n   El contenido es el mismo, no se necesita crear duplicado.");
    return;
  }

  console.log("Creando post de Agentes AI...");

  const post = await db.post.create({
    data: {
      slug: "Que-es-un-agente-AI_elw",
      title: "¿Qué es un agente AI?",
      body: agentesAIPostContent.trim(),
      published: true,
      ...
      // Clasificación
      tags: ["openai", "llm", "ai agent", "explicación", "go", "webrtc"],
      mainTag: "ai",
    },
  });
